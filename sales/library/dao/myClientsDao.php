...
class myClientsDao extends Dao
{
     ...
     * @return array
     */
    public function getFriends($friendIds,$page,$length,$where){
        $sql = "select d.deal_id,h.UsrName,h.UsrMp,d.title,o.order_money,o.OrdDate,d.start_date,d.end_date,o.`status`,d.expires_type,d.expires FROM cp_user_yaoqingma_list y left join cp_deal_order o on y.friends = o.uid left join cp_deal d on o.deal_id = d.deal_id right join cp_user_huifu h on y.friends = h.uid where o.`status`=2 and y.friends in($friendIds) $where limit $page ,$length";
     ...
     * @return type
     */
    public function getFriendsCount($friendIds,$where){
