...
//        }
    ...
    public function upload($key='') {
        ...
        foreach($files as $key =>$file) {
            ...
            $file['savename'] = $saveRuleFunc( $file['name'] ) . '.' . $file['extension'];
    ...
    protected function check($file)
    {
        ...
        //检查是否合法上传
        $file['tmp_name'] = stripslashes($file['tmp_name']);
    ...
    protected function thumb($file) {
        if ($this->thumb && in_array($file['extension'], array('gif', 'jpg', 'jpeg', 'bmp', 'png')) ) {
            ...
            require_once(dirname(__FILE__).'/image.init.php');
            $imagename =  isset($this->saeStorage) ? $file['tmp_name'] : $file['savepath'] . $file['savename'];
            return imageInit::thumb($imagename, $thumbname, $this->domain, $this->thumbMaxWidth, $this->thumbMaxHeight); // 生成图像缩略图
        }
        return false;
    }
//
...
//    }
    ...
    protected function localSave($srcFileName, $destFileName) {
        ...
        $srcFileName = stripslashes($srcFileName);
        $destFileName = stripslashes($destFileName);
