...
class baseController extends Controller
{
    ...
    public function __construct() {
        parent::__construct();
        $this->loginService = InitPHP::getService("login");//获取Service
        $isLogin = $this->loginService->isLogin();
        if($isLogin==false)
        {
            $this->controller->redirect("/login/run");
        }
    }
    /**
     * 分页
     */
    public function pages($listRows,$url,$count)
    {
        ...
        $startCount=($page-1)*$listRows; //分页开始
        $limit = $startCount.','.$listRows;

        $data = $pager->pager($count, $listRows, $url, true); //最后一个参数为true则使用默认样式
        return array('data'=>$data,'limit'=>$limit);
    }
    /************************************************************
